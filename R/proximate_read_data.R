...
#' \code{\link[base]{textConnection}} can also be passed.
...
proximate_read_data <- function(file) {
  if (all(class(file) %in% c("file", "connection"))) {
    ss <- readLines(file)
    tt <- paste0(ss, collapse = "\r\n")
    file <- textConnection(tt)
    on.exit(try(close(file), silent = TRUE), add = TRUE)
    rm(ss)
  }
  # Read the first line (& header)
  first_row <- readLines(file, n = 2) |> iconv(from = "", to = "UTF-8", sub = "byte")
  if (inherits(file, "textConnection")) {
    # push back the lines that were read for first row
    pushBack(tail(first_row, 1), file, newLine = TRUE, encoding = "UTF-8")
    pushBack(head(first_row, 1), file, newLine = TRUE, encoding = "UTF-8")
  }
  fin_entry <- tail(strsplit(tail(first_row, 1), "\t")[[1]], 1)
