...
sample_stratified <- function(y, p, number, group = NULL, replacement = FALSE, seed = NULL) {
  ...
  if (is.null(group)) {
    ...
    # Must differentiate if seed is NULL or not, as with_seed throws an error
    # when seed is NULL.
    if (is.null(seed)) {
      eval(stratified_code)
    } else {
      withr::with_seed(seed, eval(stratified_code))
    }
  } else {
    ...
    group_code <- expression({
      ...
      for (jj in 1:number) {
        ...
        if (replacement) {
          sel_sample_indices <- lapply(seq_along(strata_samples$calibration),
    ...
    if (is.null(seed)) {
      ...
      withr::with_seed(seed, eval(group_code))
    }
    ...
    if (is.null(seed)) {
      eval(replacement_code)
    } else {
      withr::with_seed(seed, eval(replacement_code))
    }
  }
...
#' @return a list with two data frames: \code{sample_strata} contains
...
#' @param y the vector of reference values
  ...
  strata_samples <- lapply(levels(strata),
    FUN = function(strata,
      ...
      if (replacement) {
        ...
        vec_samples[seq_along(ith_sel)] <- ith_sel
      }
      vec_samples
    },
    strata = strata,
